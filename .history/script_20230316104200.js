...
// '[min = 1, j = 1, 2, 3, 4, 5, 6\n]' POSITION
// 
function exo17(abuleplusbeau){
    let n = abuleplusbeau.length;
  // n = 7 - 1 caracteres 
    for (let i = 0; i < n  ; i++){
        let min = i;
        for (let j = i + 1; j < n; j++){
            if (abuleplusbeau[j] < abuleplusbeau[min]){
                min = j;
            }
        }
        if (min !== i){
        let temp = abuleplusbeau[i];
        abuleplusbeau[i] = abuleplusbeau[min];
        abuleplusbeau[min] = temp;
        }
    }
    console.log(abuleplusbeau);
}
